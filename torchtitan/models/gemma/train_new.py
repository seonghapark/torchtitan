...
Launch (single node, 8 devices):
    torchrun --nproc_per_node=8 -m torchtitan.models.gemma.train_new \
...
from distribution_mpi4py import is_rank0 as mpi_is_rank0
from distribution_mpi4py import setup_distributed as mpi_setup_distributed
...
def main(cfg: GemmaSFTConfig) -> None:
    ...
    _log(f"[model] moving to {device}")
    model.to(device)
    _log("[model] moved to device")

    _log(f"[fsdp] initializing device mesh device_type={device.type} world_size={world_size}")
    mesh = init_device_mesh(
        device_type=device.type, mesh_shape=(world_size,), mesh_dim_names=("fsdp",)
    )
    _log("[fsdp] device mesh initialized")
    _log("[fsdp] applying FSDP2")
    ...
    _log("[fsdp] FSDP2 applied")
    model.train()

    # ------------------------ Optimizer + schedule --------------------
    _log("[optimizer] creating AdamW")
